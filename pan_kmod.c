...
static const struct {
   ...
   {
      /* Arm mali_kbase proprietary KMD (Android / OEM kernels).
       * The /dev/mali0 device does not go through libdrm, so drmGetVersion()
       * will fail.  Detection is handled separately in
       * pan_kmod_dev_create_kbase() below.
       */
      "mali_kbase",
      &kbase_kmod_ops,
   },
                    ...
                    const struct pan_kmod_allocator *allocator)
{
   ...
   if (!version) {
      /*
       * drmGetVersion() fails on /dev/mali0 because mali_kbase is not a DRM
       * driver.  Try the kbase backend directly.
       */
      drmVersion fake_ver = {
         .version_major = 0,
         .version_minor = 0,
